...
def ecplaza_ranking():
    data_frame = pd.read_json(r'ecplaza.json', lines=True)
    data_frame = data_frame.dropna()
    matrix = []
    alter =[]
    for i in range(len(data_frame)):
        alter.append(data_frame.iloc[i,6])
        m = []
    ...
    criteria = ["moq","min_price","max_price","company year"], # attribute/column name
    weights =  [0.15,0.35,0.25,0.25]    # weights for each attribute (based on google form data collection)
    )
    ...
    with open('ecplaza_rank.json', 'w') as f:
        f.write(' ')
    data_frame.to_json(r'ecplaza_rank.json',orient = 'records')
